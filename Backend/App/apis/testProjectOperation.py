...
class TPOperation(Resource):
    ...
    # 下载pdf报告
    def get(self):
        url = os.path.join(BackendPath(), 'APP', 'data', 'Logs', request.args['tPid'],
                           'report-v' + request.args['n'] + '.pdf')  # 报告路径
        # 检查文件是否存在
        return jsonify({'success': os.path.exists(url), **({'url': url} if os.path.exists(url) else {})})
