...
class TPOperation(Resource):
    ...
    def post(self):
        ...
        # 执行实验命令
        command = r'cd /d E:\GreatLibrarian && conda run -n ' + current_app.config[
            'conda_env'] + ' poetry run gltest --testcase_path=' + testcase_path \
        ...
        try:
            thread = threading.Thread(target=lambda: run_command(command))
            thread.start()
